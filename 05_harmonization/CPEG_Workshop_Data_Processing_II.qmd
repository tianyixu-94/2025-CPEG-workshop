...
After filtering, we have 114 fossil occurrences and 2501 modern occurrences to potentially work with. How might we deal with this unequal sampling?
...
```{r}
# Practice cleaning the dataset here!
```


### 4. Merging

Now we come to the fun part. How do we align these data, which were sampled in different ways with different temporal and spatial resolutions and coverage?

Let's begin by returning to the structure of each dataset. Note that the header names differ, and that the datasets contain variables reported in different ways (units, formats) and with different resolutions.
```{r}
...
**Reassemble into your discussion groups and discuss the following questions:**
...
OK, so now we have some temporal bins to facilitate our comparisons over time with the climatic data.


#### Spatial Bins

We also observed that the spatial resolution of the climatic data differed between the past and present. Additionally, the uncertainties in the coordinates for our modern and fossil occurrences will differ, especially since we'll need to rely on plate models to palaeorotate the geographic positions of our fossil occurrences. How might we deal with this?
...
range(scotese_temp_20$Aquitanian)
diff(range(scotese_temp_20$Aquitanian))

# Burdigalian/Langhian range (20-15 Ma)
range(scotese_temp_15$Burdigalian)
diff(range(scotese_temp_15$Burdigalian))

# Modern range
range(modern_temp)
diff(range(modern_temp))
```
...
palaeo_temp <- rbind(data.frame(Bin = rep("Burdigalian", length(scotese_temp_15$Burdigalian)), #
                      ...
                      data.frame(Bin = rep("Modern", length(modern_temp)), 
                                  Temperature = modern_temp))
