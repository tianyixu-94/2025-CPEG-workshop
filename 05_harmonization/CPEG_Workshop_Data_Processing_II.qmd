...
format: html
editor: visual
author: 'Erin M. Dillion & Christopher D. Dean'
date: 2025-07-27
...
In this section, we're going to walk you through how to combine and use different datasets from deep-time and the modern to answer questions that cross the palaeo-ecological divide. We'll cover:

1.  Understanding the power of harmonizing data from different sources.
2.  The considerations and complications that have to be made when combining datasets.
3.  Sourcing and using spatially explicit climatic data for the past and modern.
4.  How to plan, write code for, and carry out projects covering multiple time frames.

To do this, we'll continue using the Crocodylia data we've been working with earlier today. If you've not quite finished working on your dataset, you can find a cleaned version that's ready for this next step **HERE**.

## Plan for the Session

1.  Introduction to harmonization and synthesis
2.  Planning, Setup, and Finding Your Data
3.  Merging Datasets: Building a Workflow
4.  Data Visualisation and Synthesis
...
Before doing anything else, we first have to define our research question, which will be based around the dataset of Cenozoic Crocodylia that we've been working on this morning. As mentioned previously, Crocs are an excellent group to use for studies comparing the modern and deep-time records: they have a very tight association with temperature, are well represented in the fossil record spatially and temporally, and many extant species require conservation due to anthropogenic impacts.

In this section, we'll be trying to answer the following research question: **How does the temperature range inhabited by Crocodylia in deep-time compare to that seen in the present?**

### Making a plan

Before we start, it's helpful to write out a step-by-step plan of what we want to accomplish to ensure that the approach we're taking and the code that we write will appropriately answer the question(s) that we are interested in. Think of this as a kind of "logical flow": what are all the steps needed to get from our research idea to our finalised analysis. This can be as extensive as you wish and can be done in any fashion---even writing out your code on pen and paper! We'll be doing this a few times today, so let's start by considering our research question and what materials we will need to resolve it.

As we will be comparing the distributions of Crocodylia in deep-time compared to the present, there are some key basic datasets that we will require:

1.  A spatially explicit dataset of modern Crocodylia
2.  A spatially explicit dataset of Crocodylia in the deep-time intervals we are assessing.
...
Even at this first step, we will have to start making decisions that will have consequences for our research and findings. The choice of dataset and how it's handled can have an impact on the results that we eventually obtain.
...
We'll now walk through an example for how we might acquire and harmonize our climatic data. First, let's make a plan of the steps we 'll have to take and some of the questions that we'll have to consider along the way.
...
7.  Check the format of the data using the 'class()' function.
8.  Make a reference raster with appropriate resolution and extent.
9.  Ensure climate raster doesn't have wrapping/extent issues, and rotate if necessary.
10. Re-sample raster to our reference raster.
11. Ensure that all rasters are capturing the same climatic information.

Now that we have a basic plan, we can start to put it into action. Let's write some code!

### 0. Set-up

First, we'll need to install and load some packages that will allow us to format and organise our data appropriately:
...
As we can see here, this modern data is considerably higher resolution than what we have available for our deep-time data. What issues might this cause? What actions could we take to resolve this? What could we test?
